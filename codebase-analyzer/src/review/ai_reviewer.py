...
class AIReviewer:
    ...
    def review_files(self, files: List[FileInfo], progress_callback=None) -> List[Finding]:
        ...
        self.logger.info(f"Selected {total} files for AI review")

        all_findings = []

        for idx, file_info in enumerate(selected_files, 1):
            if progress_callback:
                progress_callback(f"AI reviewing {file_info.relative_path.name}", idx, total)
            try:
