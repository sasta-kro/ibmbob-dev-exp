...
class AnalysisOrchestrator:
    ...
    def analyze_project(
        ...
        self._update_progress(f"Found {file_count} files to analyze", 10, 100)

        # Step 2: Analyze files
        self._update_progress(f"Analyzing {file_count} source files...", 20, 100)
        analyzed_files = self._analyze_files(scan_result.files, use_cache)
        self._update_progress("Code structure mapped", 40, 100)

        # Step 3: AI-powered analysis (if enabled)
        if use_ai and self.watson_service.is_available():
            self._update_progress(f"AI reviewing {file_count} files...", 50, 100)
            analyzed_files = self._enhance_with_ai(analyzed_files, use_cache)
            self._update_progress("AI analysis complete", 65, 100)
        else:
            logger.info("Skipping AI analysis (disabled or unavailable)")

        # Step 4: Group files by functionality
        self._update_progress("Grouping by functionality...", 70, 100)
        functionality_groups = self.grouper.group_files(analyzed_files, str(project_path_obj))
        self._update_progress(f"Found {len(functionality_groups)} feature groups", 80, 100)

        # Step 5: Detect frameworks and technologies
        self._update_progress("Detecting frameworks & tech stack...", 85, 100)
        frameworks = self.scanner.detect_framework(str(project_path_obj))
        if frameworks:
            self._update_progress(f"Detected: {', '.join(list(frameworks.keys())[:3])}", 90, 100)
    ...
    def generate_documentation(
        ...
        self._update_progress("Generating documentation...", 0, 100)
        ...
        self._update_progress(f"Writing docs for {file_count} files...", 20, 100)
        doc_files = self.markdown_generator.generate_project_documentation(
            project, output_path, progress_callback=self.progress_callback
        )

        self._update_progress("Generating API reference...", 60, 100)
        ...
        self._update_progress("Building documentation index...", 85, 100)
        index_path = output_path / 'INDEX.md'
        self.index_generator.generate_index(project, doc_files, index_path)

        self._update_progress(f"Documentation complete — {len(doc_files)} files generated", 100, 100)
    ...
    def review_project(
        ...
        self._update_progress("Starting code review...", 0, 100)
        ...
        self._update_progress("Running static analysis...", 30, 100)
        ...
        finding_count = review_result.get('total_findings', 0)
        self._update_progress(f"Review complete — {finding_count} findings", 100, 100)
        logger.info(f"Code review complete: {finding_count} findings")
    ...
    def _analyze_files(
        ...
        for idx, file_info in enumerate(files, 1):
            self._update_progress(
                f"Analyzing {file_info.relative_path.name}",
                idx, total
            )
    ...
    def _enhance_with_ai(
        ...
        for idx, file_info in enumerate(files, 1):
            if file_info in important_files:
                self._update_progress(
                    f"AI reviewing {file_info.relative_path.name}",
                    idx, total
                )
                try:
